...
BrnBuffer::~BrnBuffer() {
  unmap();
  vkDestroyBuffer(brnDevice.device(), buffer, nullptr);
  vkFreeMemory(brnDevice.device(), memory, nullptr);
}
 ...
 * @return VkResult of the buffer mapping call
 */
VkResult BrnBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
  assert(buffer && memory && "Called map on buffer before create");
  return vkMapMemory(brnDevice.device(), memory, offset, size, 0, &mapped);
}
 ...
 */
void BrnBuffer::unmap() {
  if (mapped) {
    vkUnmapMemory(brnDevice.device(), memory);
    mapped = nullptr;
  }
}
 ...
 */
VkResult BrnBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = memory;
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkFlushMappedMemoryRanges(brnDevice.device(), 1, &mappedRange);
}
 ...
 */
VkResult BrnBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = memory;
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(brnDevice.device(), 1, &mappedRange);
}
 ...
 *
 */
void BrnBuffer::writeToIndex(void *data, int index) {
 ...
 *
 */
VkResult BrnBuffer::flushIndex(int index) {
 ...
 */
VkDescriptorBufferInfo BrnBuffer::descriptorInfoForIndex(int index) {
 ...
 * @return VkResult of the invalidate call
 */
VkResult BrnBuffer::invalidateIndex(int index) {
